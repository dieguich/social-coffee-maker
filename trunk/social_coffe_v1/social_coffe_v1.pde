...
void setup(){
  ...
  if(digitalRead(waterLevelPin) == HIGH){
    analogWrite(GreenPin, y); //initialize pins to 255 (switch on)
    analogWrite(RedPin, x);   //initialize pins to 0 (switch off)  
  }
  else{
    analogWrite(GreenPin, x); //initialize pins to 0 (switch off)
    analogWrite(RedPin, y);   //initialize pins to 255 (switch on)
    coffeIsFull = false;  
  }
...
void controlCoffeMade(){
  ...
  if ((readCurrentValue == 0) && (currentIsFlowing)){ //If the machine is idle and it was working in the previous loop.
      ...
      timeOn = millis()-timeCount-2000; //Compute the time used to prepare a coffee (2secons to establish the current to 0 after a hot drink made) 
